...
class P0P1FinalClosureRetest:
    ...
    def test_p0_canonical_endpoints(self):
        ...
        try:
            # Test key P0 endpoints
            endpoints = [
                {
                    "path": "/api/audit-logs/trading-lifecycle",
                    "method": "GET",
                    "required_fields": ["chains", "has_more", "query_latency_ms"]
                },
                {
            ...
            if len(passed_endpoints) == len(endpoints):
                self.log_result("P0 Canonical Endpoints", "PASS", f"All endpoints working: {', '.join(passed_endpoints)}")
            elif len(passed_endpoints) > 0:
                self.log_result("P0 Canonical Endpoints", "PARTIAL", f"{len(passed_endpoints)}/{len(endpoints)} working: {', '.join(passed_endpoints)}. Issues: {', '.join(failed_endpoints)}")
            else:
    ...
    def test_p1_features(self):
        ...
        try:
            ...
            if len(features_tested) == total_features and total_features > 0:
                self.log_result("P1 Features", "PASS", f"All features working: {', '.join(features_tested)}")
            elif len(features_tested) > 0:
                self.log_result("P1 Features", "PARTIAL", f"{len(features_tested)}/{total_features} working: {', '.join(features_tested)}. Issues: {', '.join(features_failed)}")
            else:
                self.log_result("P1 Features", "FAIL", f"No P1 features working. Issues: {', '.join(features_failed)}")
    ...
    def test_explain_replay_contracts(self):
        ...
        try:
                ...
                try:
                    if explain_response.status_code == 200:
                        ...
                        if not missing:
                            ...
                            contracts_failed.append(f"explain-contract(missing:{','.join(missing)})")
                    else:
                        contracts_working.append("explain-endpoint")  # Endpoint exists
                except:
                    contracts_failed.append("explain-contract(parse-error)")
            else:
            ...
            if len(contracts_working) == total_contracts and total_contracts > 0:
                self.log_result("Explain/Replay Contracts", "PASS", f"All contracts working: {', '.join(contracts_working)}")
            elif len(contracts_working) > 0:
                self.log_result("Explain/Replay Contracts", "PARTIAL", f"{len(contracts_working)}/{total_contracts} working: {', '.join(contracts_working)}. Issues: {', '.join(contracts_failed)}")
            else:
                self.log_result("Explain/Replay Contracts", "FAIL", f"No contracts working. Issues: {', '.join(contracts_failed)}")
    ...
    def run_all_tests(self):
        ...
        print("=" * 80)
        print("P0+P1 FINAL CLOSURE RETEST (POST-SECURITY-HEADER FIX)")
