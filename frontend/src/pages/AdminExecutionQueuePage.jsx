      ...
      <div className="overflow-x-auto border border-slate-800 bg-slate-900" data-testid="admin-execution-queue-table-wrapper">
        <table className="min-w-full text-sm" data-testid="admin-execution-queue-table" aria-label="Execution queue tablosu">
          ...
          <tbody data-testid="admin-execution-queue-table-body">
            {queueRows.map((row) => (
              <tr key={row.id} className="border-t border-slate-800" data-testid="admin-execution-queue-row">
                <td className="px-3 py-2" data-testid="admin-execution-queue-row-intent">{row.id}</td>
                <td className="px-3 py-2" data-testid="admin-execution-queue-row-user">{row.user_email || row.user_id}</td>
                <td className="px-3 py-2">{row.symbol}</td>
                <td className="px-3 py-2">{row.market_type}</td>
                <td className="px-3 py-2">{row.side}</td>
                <td className="px-3 py-2">{row.notional}</td>
                <td className="px-3 py-2">{row.status}</td>
                <td className="px-3 py-2">{(row.risk_flags || []).join(", ") || "-"}</td>
                <td className="px-3 py-2">
                  {row.status === "QUEUED" ? (
                    <div className="flex gap-2" data-testid="admin-execution-queue-actions">
                      <Button className="bg-emerald-500 text-black hover:bg-emerald-400" onClick={() => decide(row.id, "approve")} data-testid="admin-execution-queue-approve-button">Approve</Button>
                      <Button variant="outline" onClick={() => decide(row.id, "reject")} data-testid="admin-execution-queue-reject-button">Reject</Button>
                    </div>
                  ) : (
