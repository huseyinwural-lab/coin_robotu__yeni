#!/usr/bin/env python3
"""
FAZ-3 Drift Action Center Backend Validation Test - CORRECTED
...
class DriftActionTester:
    ...
    def test_mute_duration_validation(self, alert_id: str):
        ...
        invalid_duration = 2
        payload = {"duration_hours": invalid_duration, "reason": "Test invalid mute"}
        try:
            headers = {"Authorization": f"Bearer {self.super_admin_token}"}
            ...
            if response.status_code == 400 or response.status_code == 422:
                ...
                data = response.json()
                if data.get("status") == "error" or "invalid" in data.get("message", "").lower():
                    self.log_test(f"Mute duration {invalid_duration}h validation", True, "Invalid duration rejected in response")
                else:
    ...
    def test_confirm_enforcement(self, alert_id: str):
        ...
        try:
            ...
            if response.status_code == 400 or response.status_code == 422:
                ...
                data = response.json()
                if data.get("status") == "error" or "confirmation" in data.get("message", "").lower():
                    self.log_test("Ignore confirmation enforcement", True, "Wrong confirmation rejected in response")
                else:
