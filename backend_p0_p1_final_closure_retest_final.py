...
class P0P1FinalClosureRetestFinal:
    def __init__(self):
            ...
            if response.status_code == 200:
                ...
                for metric in required_metrics:
                    if metric == "latency":
                        ...
                        latency_matches = re.findall(r'(\w*latency\w*)\{.*?\}\s+(\S+)', metrics_text, re.IGNORECASE)
                        if latency_matches:
                            latency_info = ", ".join([f"{name}={value}" for name, value in latency_matches[:2]])  # Show first 2
                            found_metrics.append(f"latency({latency_info})")
                        else:
                            missing_metrics.append("latency")
                    else:
                        # Look for exact metric name
                        pattern = rf'^{re.escape(metric)}\{{.*?\}}\s+(\S+)'
    ...
    def run_all_tests(self):
        ...
        print("=" * 80)
        print("P0+P1 FINAL CLOSURE RETEST (POST-SECURITY-HEADER FIX) - FINAL")
...
def main():
    ...
    tester = P0P1FinalClosureRetestFinal()
