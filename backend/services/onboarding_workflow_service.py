...
from models import User, UserOnboardingWorkflowCase, UserOnboardingWorkflowStepLog, UserRole
...
def list_priority_queue(db: Session, *, assigned_admin_id: str | None = None) -> list[dict]:
    ...
    rows = query.order_by(UserOnboardingWorkflowCase.priority_score.desc(), UserOnboardingWorkflowCase.sla_due_at.asc()).all()
    payload = []
    for row in rows:
        payload.append(
            {
