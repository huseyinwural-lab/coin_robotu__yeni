"""
Iteration 142 - Closure Testing: Infra stabilization, smoke PASS, readiness READY, final go/no-go artifact, env hardening, final regression.
...
class TestWizardAuthSuperAdminOnly:
    ...
    @pytest.fixture
    def regular_user_token(self, admin_token):
        """Create a regular user and get token (or skip if not possible)"""
        # For this test, we'll test that non-super_admin gets 403
        # We can test by checking the endpoint behavior
        pytest.skip("Regular user creation not implemented - testing with admin token only")

    def test_wizard_readiness_check_requires_super_admin(self, admin_token):
        """POST /api/runtime/go-live/wizard/readiness-check requires super_admin"""
        headers = {"Authorization": f"Bearer {admin_token}"}
        
        # With super_admin token, should get 200 (not 403)
        response = requests.post(
    ...
    def test_wizard_canary_check_requires_super_admin(self, admin_token):
        """POST /api/runtime/go-live/wizard/canary-check requires super_admin"""
        headers = {"Authorization": f"Bearer {admin_token}"}
        ...
        # Should NOT be 403 for super_admin
        assert response.status_code != 403, "super_admin should not get 403"
        print(f"✓ wizard/canary-check: super_admin gets {response.status_code}")

    def test_wizard_arm_requires_super_admin(self, admin_token):
    ...
    def test_wizard_confirm_requires_super_admin(self, admin_token):
    ...
    def test_wizard_rollback_requires_super_admin(self, admin_token):
