...
def sync_all_positions_for_user(db: Session, user_id: str) -> list[Position]:
    cluster_mapping = _cluster_map(db)
    rows = db.query(PaperPosition).filter(PaperPosition.user_id == user_id).all()
    for paper in rows:
        sync_position_state(
            db,
...
def list_user_positions(db: Session, user_id: str, include_closed: bool = False) -> list[Position]:
    sync_all_positions_for_user(db, user_id)
