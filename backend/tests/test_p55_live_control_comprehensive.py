"""
Phase 5.5 Live Control Comprehensive Test Suite
Tests: execution contract, live adapter, preflight, retry policy, 
...
# Live Adapter Tests
# ============================================================================
class TestFuturesLiveAdapter:
    """Live adapter (BinanceFuturesLiveAdapter wrapper) testleri"""
    
    def test_adapter_instantiation(self):
        from core.execution.futures_live_adapter import FuturesLiveAdapter
        adapter = FuturesLiveAdapter()
...
class TestFuturesOrderPreflight:
    ...
    def test_preflight_blocks_when_live_disabled(self):
        from core.execution.futures_execution_contract import FuturesExecutionRequest
        from core.execution.futures_order_preflight import FuturesOrderPreflight
        req = FuturesExecutionRequest(
            symbol="BTCUSDT",
        ...
        result = FuturesOrderPreflight().evaluate(req, context)
        assert result["preflight_pass"] is False
        live_check = next((c for c in result["checks"] if c["key"] == "live_mode_enabled"), None)
        assert live_check is not None
        assert live_check["pass"] is False
...
# Release Gate Tests
# ============================================================================
class TestFuturesLiveReleaseGate:
    """Live release gate testleri"""
    ...
    def test_release_gate_blocked_when_disabled(self):
        ...
        assert "LIVE_MODE_DISABLED_BY_DEFAULT" in result["reasons"]
...
class TestFuturesExecutionParityCheck:
    """Parity check (paper vs live) testleri"""
...
# ============================================================================
class TestAdminLiveEndpoints:
    """Admin live control endpoint testleri"""
    
    def test_live_status_endpoint_200(self, admin_token):
        headers = {"Authorization": f"Bearer {admin_token}"}
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/status", headers=headers, timeout=25)
        assert response.status_code == 200

    def test_live_status_contract_fields(self, admin_token):
        headers = {"Authorization": f"Bearer {admin_token}"}
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/status", headers=headers, timeout=25)
        payload = response.json()
        required_fields = [
            "default_mode", "live_enabled", "live_endpoint_access", 
    ...
    def test_live_status_default_mode_paper(self, admin_token):
        headers = {"Authorization": f"Bearer {admin_token}"}
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/status", headers=headers, timeout=25)
        payload = response.json()
        assert payload["default_mode"] == "paper"

    def test_live_status_live_endpoint_access_false(self, admin_token):
        headers = {"Authorization": f"Bearer {admin_token}"}
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/status", headers=headers, timeout=25)
        payload = response.json()
    ...
    def test_release_gate_endpoint_200(self, admin_token):
        headers = {"Authorization": f"Bearer {admin_token}"}
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/release-gate", headers=headers, timeout=25)
        assert response.status_code == 200
    ...
    def test_release_gate_contract_fields(self, admin_token):
        ...
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/release-gate", headers=headers, timeout=25)
        payload = response.json()
        required_fields = ["status", "order_path_open", "reasons", "secret_isolation", "live_enabled"]
        for field in required_fields:
            assert field in payload, f"Missing field: {field}"
    ...
    def test_release_gate_order_path_blocked_by_default(self, admin_token):
        ...
        response = requests.get(f"{BASE_URL}/api/admin/futures/live/release-gate", headers=headers, timeout=25)
        payload = response.json()
        # Gate should be blocked when live is disabled by default
