...
LOG_FILE="${ARTIFACT_DIR}/canary_c3_run.log"
SUMMARY_JSON="${ARTIFACT_DIR}/canary_c3_summary.json"
METRICS_JSON="${ARTIFACT_DIR}/canary_c3_metrics_snapshot.json"
...
CANARY_SYMBOLS_JSON='["BTCUSDT","ETHUSDT","BNBUSDT","SOLUSDT","XRPUSDT","ADAUSDT","DOGEUSDT","LTCUSDT","LINKUSDT","DOTUSDT"]'
CANARY_SYMBOLS=("BTCUSDT" "ETHUSDT" "BNBUSDT" "SOLUSDT" "XRPUSDT" "ADAUSDT" "DOGEUSDT" "LTCUSDT" "LINKUSDT" "DOTUSDT")

CANARY_CAPITAL="${CANARY_C3_CAPITAL_USDT:-900}"
if ! python - <<PY
v=float('${CANARY_CAPITAL}')
if 800 <= v <= 1000:
    raise SystemExit(0)
raise SystemExit(1)
PY
then
  fail "CANARY_C3_CAPITAL_USDT 800-1000 aralığında olmalı (current=${CANARY_CAPITAL})"
fi

CANARY_MAX_POSITIONS="5"
RUN_MINUTES="${CANARY_C3_RUN_MINUTES:-60}"
if ! python - <<PY
v=int('${RUN_MINUTES}')
if 60 <= v <= 90:
    raise SystemExit(0)
...
PY
then
  fail "CANARY_C3_RUN_MINUTES 60-90 aralığında olmalı (current=${RUN_MINUTES})"
fi
...
# Kritik gözlem guard'ları
ORDER_QUEUE_DEPTH_LIMIT=200
ORDER_QUEUE_GROWTH_STREAK_LIMIT=4
PARALLEL_QUEUE_DEPTH_LIMIT=60
PARALLEL_CYCLE_LATENCY_LIMIT=4500
...
login_admin() {
  local code
  code="$(request_json POST "${BASE_URL}/api/auth/login/admin" "{\"email\":\"${ADMIN_EMAIL}\",\"password\":\"${ADMIN_PASSWORD}\"}" "" "/tmp/c3_admin_login.json")"
  [[ "${code}" == "200" ]] || fail "admin login başarısız http=${code}"
  ADMIN_TOKEN="$(extract_token /tmp/c3_admin_login.json)"
...
ensure_user_login() {
  ...
  if [[ "${login_code}" != "200" ]]; then
    local reg_code
    reg_code="$(request_json POST "${BASE_URL}/api/auth/register" "{\"email\":\"${USER_EMAIL}\",\"password\":\"${USER_PASSWORD}\",\"first_name\":\"Canary\",\"last_name\":\"C3\"}" "" "/tmp/c3_user_register.json")"
    [[ "${reg_code}" == "200" ]] || fail "user register başarısız http=${reg_code}"

    local pending_code
    pending_code="$(request_json GET "${BASE_URL}/api/auth/admin/user-approval-requests" "" "${ADMIN_TOKEN}" "/tmp/c3_pending_users.json")"
    [[ "${pending_code}" == "200" ]] || fail "pending approvals alınamadı"

    USER_ID="$(python - <<PY
import json
rows=json.load(open('/tmp/c3_pending_users.json', encoding='utf-8'))
...
PY
  ...
  USER_TOKEN="$(extract_token /tmp/c3_user_login.json)"
...
set_exchange_keys() {
  ...
  code="$(request_json PUT "${BASE_URL}/api/phase4/exchange-settings" "${payload}" "${USER_TOKEN}" "/tmp/c3_exchange_settings.json")"
...
load_live_config() {
  local code
  code="$(request_json GET "${BASE_URL}/api/phase4/live-config" "" "${ADMIN_TOKEN}" "/tmp/c3_live_get.json")"
...
update_live_config_for_symbol() {
  ...
  python - <<PY
import json
cfg=json.load(open('/tmp/c3_live_get.json', encoding='utf-8'))
cfg.update({
  'canary_enabled': True,
...
open('/tmp/c3_live_put.json','w',encoding='utf-8').write(json.dumps(cfg))
PY
  local payload code
  payload="$(cat /tmp/c3_live_put.json)"
  code="$(request_json PUT "${BASE_URL}/api/phase4/live-config" "${payload}" "${ADMIN_TOKEN}" "/tmp/c3_live_put_resp.json")"
  [[ "${code}" == "200" ]] || fail "live-config update başarısız http=${code}"
}

fetch_canary_status() {
  local code
  code="$(request_json GET "${BASE_URL}/api/admin/canary-status" "" "${ADMIN_TOKEN}" "/tmp/c3_canary_status.json")"
  [[ "${code}" == "200" ]] || fail "canary-status başarısız http=${code}"
}

fetch_execution_queue_summary() {
  local code
  code="$(request_json GET "${BASE_URL}/api/admin/execution-queue/rejection-summary" "" "${ADMIN_TOKEN}" "/tmp/c3_execution_queue.json")"
  [[ "${code}" == "200" ]] || fail "execution-queue/rejection-summary başarısız http=${code}"
}

fetch_universe_monitor() {
  local code
  code="$(request_json GET "${BASE_URL}/api/admin/universe-monitor?market_type=futures" "" "${ADMIN_TOKEN}" "/tmp/c3_universe_monitor.json")"
  [[ "${code}" == "200" ]] || fail "admin/universe-monitor başarısız http=${code}"
}

test_order() {
  request_json POST "${BASE_URL}/api/phase4/test-order" "{}" "${USER_TOKEN}" "/tmp/c3_test_order.json"
}
...
kill_switch_toggle() {
  ...
  local reason="$2"
  local code
  code="$(request_json POST "${BASE_URL}/api/admin/kill-switch" "{\"trading_enabled\":${trading_enabled},\"reason\":\"${reason}\"}" "${ADMIN_TOKEN}" "/tmp/c3_kill_switch.json")"
  [[ "${code}" == "200" ]] || fail "kill-switch toggle başarısız (${reason}) http=${code}"
}

log "C3 START config: symbols=10 capital=${CANARY_CAPITAL} max_positions=5 duration=${RUN_MINUTES}m"

HCODE="$(curl -sS -o /tmp/c3_health_gate.json -w '%{http_code}' "${BASE_URL}/api/health" || true)"
RCODE="$(curl -sS -o /tmp/c3_ready_gate.json -w '%{http_code}' "${BASE_URL}/api/ready" || true)"
...
fetch_execution_queue_summary
fetch_universe_monitor
log "PASS: c3 başlangıç status endpoint kontrolleri"

log "Kill-switch faz başı testi"
kill_switch_toggle false "c3_phase_start_kill_test"
...
python - <<PY
import json
body=json.load(open('/tmp/c3_test_order.json', encoding='utf-8'))
...
PY
kill_switch_toggle true "c3_phase_start_resume"
...
import json
status=json.load(open('/tmp/c3_canary_status.json', encoding='utf-8'))
queue=json.load(open('/tmp/c3_execution_queue.json', encoding='utf-8'))
um=json.load(open('/tmp/c3_universe_monitor.json', encoding='utf-8'))
print('STATUS', status)
...
import json
s=json.load(open('/tmp/c3_canary_status.json', encoding='utf-8'))
...
import json
q=json.load(open('/tmp/c3_execution_queue.json', encoding='utf-8'))
...
import json
u=json.load(open('/tmp/c3_universe_monitor.json', encoding='utf-8'))
...
[[ "${DURATION_MINUTES}" -ge 60 ]] || fail "C3 süresi 60dk altında (${DURATION_MINUTES})"
[[ "${DURATION_MINUTES}" -le 90 ]] || fail "C3 süresi 90dk üstünde (${DURATION_MINUTES})"
[[ "${LOOP_COUNT}" -ge 10 ]] || fail "minimum 10 loop sağlanmadı (${LOOP_COUNT})"

log "Kill-switch faz sonu testi"
kill_switch_toggle false "c3_phase_end_kill_test"
...
PY
kill_switch_toggle true "c3_phase_end_resume"

HEALTH_CODE="$(curl -sS -o /tmp/c3_health.json -w '%{http_code}' "${BASE_URL}/health" || true)"
READY_CODE="$(curl -sS -o /tmp/c3_ready.json -w '%{http_code}' "${BASE_URL}/ready" || true)"
if [[ "${HEALTH_CODE}" != "200" ]]; then
  HEALTH_CODE="$(curl -sS -o /tmp/c3_health.json -w '%{http_code}' "${BASE_URL}/api/health" || true)"
fi
if [[ "${READY_CODE}" != "200" ]]; then
  READY_CODE="$(curl -sS -o /tmp/c3_ready.json -w '%{http_code}' "${BASE_URL}/api/ready" || true)"
fi
...
summary = {
  ...
  "symbols": ["BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT", "XRPUSDT", "ADAUSDT", "DOGEUSDT", "LTCUSDT", "LINKUSDT", "DOTUSDT"],
  "capital_usdt": float('${CANARY_CAPITAL}'),
  "capital_range": "800-1000",
  "max_positions": 5,
