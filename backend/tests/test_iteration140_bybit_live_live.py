"""
Iteration 140 - Bybit Live+Live Credential Testing

Tests:
1. Admin credential form: bybit_live_api_key/bybit_live_secret/bybit_live_api_key/bybit_live_secret fields render
2. PATCH /api/venues/admin/execution-credentials accepts new fields
3. GET /api/venues/admin/execution-credentials returns has_bybit_live_credentials + has_bybit_live_credentials
4. POST /api/venues/admin/execution-validation returns bybit_live_live_ready
...
class TestGetExecutionCredentialsHasFields:
    """Test GET /api/venues/admin/execution-credentials returns has_bybit_live_credentials + has_bybit_live_credentials"""
...
class TestExecutionValidationBybitReady:
    """Test POST /api/venues/admin/execution-validation returns bybit_live_live_ready field"""

    def test_execution_validation_returns_bybit_live_live_ready(self, admin_client):
        """POST execution-validation returns bybit_live_live_ready field"""
        response = admin_client.post(f"{BASE_URL}/api/venues/admin/execution-validation")
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        
        validation = data.get("validation", {})
        assert "bybit_live_live_ready" in validation, "validation should contain bybit_live_live_ready field"
        
        # Value can be "PASS" or "DEGRADED" depending on credential validity
        valid_values = ["PASS", "DEGRADED", "MOCKED"]
        assert validation["bybit_live_live_ready"] in valid_values, \
            f"bybit_live_live_ready should be one of {valid_values}, got {validation['bybit_live_live_ready']}"
        
        print(f"PASS: bybit_live_live_ready field present: {validation['bybit_live_live_ready']}")
