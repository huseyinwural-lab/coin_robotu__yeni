...
from models import LiveExecutionLog, UserExchangeSetting
...
def _latest_execution_row(db: Session):
    return db.query(LiveExecutionLog).order_by(LiveExecutionLog.created_at.desc()).first()
...
def build_live_release_gate_status(db: Session) -> dict:
    ...
    return {
        ...
        "live_enabled": bool(config.live_mode_enabled),
        "safe_mode_enabled": bool(config.safe_mode_enabled),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }


def build_live_status(db: Session, cache, user_id: str) -> dict:
...
def build_live_execution_quality(db: Session, cache, user_id: str) -> dict:
    decision_diagnostics = get_futures_decision_diagnostics(db, cache, user_id, refresh=False)
...
def build_live_execution_quality_rolling_7d(db: Session, cache, user_id: str) -> dict:
    decision_diagnostics = get_futures_decision_diagnostics(db, cache, user_id, refresh=False)
