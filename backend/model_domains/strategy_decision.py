...
class StrategyTemplate(Base):
    ...
    template_code: Mapped[str] = mapped_column(String(120), index=True, default=lambda: f"tpl_{uuid.uuid4().hex[:10]}")
    version_group_id: Mapped[str] = mapped_column(String(120), index=True, default=lambda: str(uuid.uuid4()))
    version_num: Mapped[int] = mapped_column(Integer, default=1)
    lifecycle_state: Mapped[str] = mapped_column(String(30), default="DRAFT")
    parent_template_id: Mapped[str | None] = mapped_column(String(120), ForeignKey("strategy_templates.id"), nullable=True)
    rollback_from_template_id: Mapped[str | None] = mapped_column(String(120), ForeignKey("strategy_templates.id"), nullable=True)
    strategy_type: Mapped[str] = mapped_column(String(50))
    parameters: Mapped[dict] = mapped_column(JSON, default=dict)
    param_schema: Mapped[dict] = mapped_column(JSON, default=dict)
    logic_schema: Mapped[dict] = mapped_column(JSON, default=dict)
    indicator_schema: Mapped[dict] = mapped_column(JSON, default=dict)
    backtest_result_ref: Mapped[str | None] = mapped_column(String(120), nullable=True)
    last_validated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    execution_profile_ref: Mapped[str | None] = mapped_column(String(120), nullable=True)
    risk_hint_ref: Mapped[str | None] = mapped_column(String(120), nullable=True)
    allowed_venues: Mapped[list[str]] = mapped_column(JSON, default=list)
